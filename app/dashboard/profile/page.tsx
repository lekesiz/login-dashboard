      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/4">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="mt-4 text-lg font-semibold text-gray-900">{user.data.name}</h2>
              <p className="text-sm text-gray-600">{user.data.email}</p>
              <Badge variant="info" className="mt-2">
                {user.data.role.displayName}
              </Badge>
            </div>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-900">
                  {user.data.emailVerified ? '✓ Doğrulandı' : '⏳ Beklemede'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Oturumlar:</span>
                <span className="text-gray-900">{user.data._count.sessions}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Aktiviteler:</span>
                <span className="text-gray-900">{user.data._count.activities}</span>
              </div>
            </div>
        ...
        <div className="flex-1">
          <div className="bg-white rounded-lg shadow">
            {activeTab === 'general' && (
              <ProfileInfoSection user={user.data} onUpdate={() => refetch()} />
            )}
