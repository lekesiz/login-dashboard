        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            <div className="h-16 w-16 rounded-full bg-blue-600 text-white flex items-center justify-center text-xl font-medium">
              {user.data.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-semibold text-gray-900">{user.data.name}</h3>
              <p className="text-gray-600">{user.data.email}</p>
              <div className="flex items-center gap-2 mt-2">
                <Badge variant={statusConfig[user.data.status as keyof typeof statusConfig].variant}>
                  {statusConfig[user.data.status as keyof typeof statusConfig].label}
                </Badge>
                <Badge variant="info">{user.data.role.displayName}</Badge>
                {user.data.emailVerified && <Badge variant="success">Email Doğrulandı</Badge>}
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">
                  {user.data.lastLogin ? formatDate(user.data.lastLogin) : 'Hiç giriş yapmadı'}
                </span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">
                  {user.data.emailVerified ? 'Doğrulandı' : 'Doğrulanmadı'}
                </span>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">{user.data._count.activities} işlem</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Users className="h-4 w-4 text-gray-400" />
                <span className="text-gray-600">Oturum:</span>
                <span className="font-medium">{user.data._count.sessions} oturum</span>
              </div>
              {user.data.invitedByUser && (
                <div className="flex items-center gap-2 text-sm">
                  <Shield className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600">Davet Eden:</span>
                  <span className="font-medium">{user.data.invitedByUser.name}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="font-medium text-gray-900">{user.data.role.displayName}</div>
              {user.data.role.description && (
                <p className="text-sm text-gray-600 mt-1">{user.data.role.description}</p>
              )}
              {user.data.role.permissions.length > 0 && (
                <div className="mt-3 space-y-1">
                  {user.data.role.permissions.map((perm) => (
          ...
          <div>
            ...
            <div className="space-y-2">
              {user.data.activities.length > 0 ? (
                user.data.activities.map((activity) => (
