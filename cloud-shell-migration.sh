...
# Cloud Shell Migration Script for Cloud SQL
# Project: bilan-competence-449414
...
# Step 2: Connect to Cloud SQL instance and run migration
echo "Connecting to Cloud SQL instance and running migration..."
echo "Note: You'll be prompted for the password for user 'modernapp'"

# Option 1: Direct execution using gcloud sql connect with psql command
gcloud sql connect bdc-db-prod --user=modernapp --database=modernwebapp << 'EOF'
-- CreateSchema
CREATE SCHEMA IF NOT EXISTS "public";

-- CreateEnum
CREATE TYPE "public"."UserStatus" AS ENUM ('ACTIVE', 'INACTIVE', 'PENDING', 'SUSPENDED', 'DELETED');

-- CreateEnum
CREATE TYPE "public"."ActivityType" AS ENUM ('AUTH_LOGIN', 'AUTH_LOGOUT', 'AUTH_FAILED', 'USER_CREATED', 'USER_UPDATED', 'USER_DELETED', 'PASSWORD_CHANGED', 'PASSWORD_RESET', 'ROLE_CHANGED', 'PERMISSION_CHANGED');
